...
export const DomainPanel: React.FC<DomainPanelProps> = ({ domain, onClose, onSymbolClick, onLoadDomain, onDomainChange }) => {
  ...
  // Fetch available domains from LOCAL CACHE
  useEffect(() => {
  ...
  const fetchPage = async (pageIndex: number, lastId: string | null) => {
    ...
    try {
      ...
      if (!result) {
          throw new Error("Domain not found or disabled in local store.");
      }
  ...
  return (
    <>
      {/* Slide-out Panel */}
      <div 
        className={`fixed inset-y-0 right-0 w-full md:w-[480px] bg-white dark:bg-gray-950 border-l border-gray-200 dark:border-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-[60] flex flex-col ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur">
            <div className="flex flex-col gap-1">
                <span className="text-[10px] uppercase tracking-widest text-gray-500 font-mono">Domain Registry (Local)</span>
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
            {loading && currentSymbols.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 gap-4 text-gray-400">
                    ...
                    <span className="font-mono text-xs"> querying local index...</span>
                </div>
            ) : error ? (
